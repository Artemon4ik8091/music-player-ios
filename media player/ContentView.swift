                                ...
                                TrackRow(track: track, isCurrent: manager.audioPlayer?.url == track.url, isPlaying: manager.isPlaying)
                                    ...
                                    .contextMenu {
                                        ...
                                        Button(role: .destructive, action: {
                                            manager.deleteTrack(track: track)
                                        }) {
                    ...
                    ToolbarItem(placement: .navigationBarTrailing) {
                        HStack(spacing: 16) {
                            ...
                            Button(action: { showSettings = true }) {
                                Image(systemName: "gearshape.fill").foregroundColor(.red).font(.title3)
                            }
                        }
                    }
                }
            ...
            Form {
                ...
                Section(header: Text("О приложении")) {
                    HStack {
                        Text("Версия")
                        Spacer()
                        Text("2.6.0").foregroundColor(.gray) // Обновлена версия
                    }
                    HStack {
                        Text("Разработчик")
                        Spacer()
        ...
        GeometryReader { geo in
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    .frame(
                        maxWidth: isImmersiveLyrics ? .infinity : (isSmall ? 220 : 300),
                        maxHeight: isImmersiveLyrics ? .infinity : (isSmall ? 220 : 300)
                    )
                    ...
                    if isImmersiveLyrics {
                        ...
                        VStack(spacing: 0) {
                            ...
                            VStack(spacing: 4) {
                                Slider(value: Binding(get: { manager.currentTime }, set: { manager.seek(to: $0) }), in: 0...max(manager.trackDuration, 1)).accentColor(.primary)
                                HStack {
                            ...
                            HStack(spacing: 12) {
                                ...
                                Slider(value: $manager.volume, in: 0...1).accentColor(.primary)
                            ...
                            HStack {
                                Button(action: manager.toggleShuffle) {
                                    Image(systemName: "shuffle").foregroundColor(manager.shuffleOn ? .red : .primary.opacity(0.6))
                                ...
                                Button(action: manager.toggleRepeat) {
                                    Image(systemName: manager.repeatMode.iconName).foregroundColor(manager.repeatMode == .none ? .primary.opacity(0.6) : .red)
